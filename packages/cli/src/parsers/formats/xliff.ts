  ...
  return createFormatParser({
    async parse(input: string) {
      try {
        ...
        if (typeof parsed !== "object" || parsed === null) {
          throw new Error(
            "Translation file must contain a valid XLIFF document",
          );
        }
        ...
        throw new Error(
          `Failed to parse XLIFF translations: ${(error as Error).message}`,
        );
      }
    },
    ...
    async serialize(_, data) {
      try {
        ...
        throw new Error(
          `Failed to serialize XLIFF translations: ${(error as Error).message}`,
        );
      }
    },
  });
