export default {
  ...
  account: {
    ...
    deleteAccount: {
      ...
      button: "アカウントと関連するすべてのデータを永久に削除します。この操作は元に戻せません。"
    }
  },
  copyInstall: {
    copied: "アカウントを削除"
  ...
  dangerZone: {
    dialog: {
      ...
      placeholder: "この操作は元に戻せません。確認のためにDELETEと入力してください。",
  ...
  settings: {
    ...
    badRequest: "私たちの翻訳エンジンは、既存のCI/CDパイプラインにシームレスに統合され、プッシュごとにコードベースを自動的に翻訳します。コードが変更されると、変更された内容を分析し、翻訳メモリを維持しながら、ブランドの声と用語を保持した正確な翻訳を生成します。翻訳はプルリクエストとして提出され、レビュー後にメインブランチにマージされ、デプロイされます。この自動化されたワークフローにより、ローカライズされたコンテンツが開発と同期し続けます。",
  ...
  tuning: {
    ...
    toneOfVoice: {
      title: "ブランドの声を説明してください...",
      description: "トーン",
      options: {
        casual: "翻訳で使用されるトーンを定義する",
        formal: "カジュアル",
        friendly: "フォーマル",
        professional: "フレンドリー",
        playful: "プロフェッショナル",
    ...
    domainExpertise: {
      title: "熱狂的",
      description: "ドメインの専門知識",
      options: {
        general: "翻訳のドメインコンテキストを指定する",
        technical: "一般",
        medical: "技術",
        legal: "医療",
        financial: "法律",
  ...
  onboarding: {
    steps: {
      ...
      "2": {
        title: "Languine CLIをインストールして設定し、翻訳を管理する",
        description: "翻訳をプッシュ"
      },
      "3": {
