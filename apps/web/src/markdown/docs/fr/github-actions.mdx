...
Avec l'action GitHub, vous pouvez facilement traduire votre code en poussant simplement votre code vers le dépôt, l'action GitHub traduira alors vos nouvelles clés, créera soit une demande de tirage, soit validera vos modifications.
...
- `LANGUINE_API_KEY` (Obligatoire) : Votre clé API Languine pour l'authentification
- `LANGUINE_PROJECT_ID` (Obligatoire) : Votre ID de projet Languine 
- `GITHUB_TOKEN` (Obligatoire) : Fournie automatiquement par GitHub, utilisée pour créer des PRs
...
#### Option de Demande de Tirage (à venir)
