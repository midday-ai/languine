...
github action을 사용하면 코드를 리포지토리에 푸시하는 것만으로 쉽게 번역할 수 있으며, github action은 새로운 키를 번역한 후 풀 리퀘스트를 생성하거나 변경 사항을 커밋합니다.
이로 인해 많은 시간과 노력을 절약하면서도 높은 품질의 번역을 유지할 수 있습니다.
...
Languine GitHub Action을 리포지토리에서 사용하려면 다음을 수행해야 합니다:
...
리포지토리에 다음 내용을 가진 .github/workflows/languine.yml 파일을 새로 생성합니다:
