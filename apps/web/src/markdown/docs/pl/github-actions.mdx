...
Dzięki akcji GitHub możesz łatwo przetłumaczyć swój kod, po prostu przesyłając go do repozytorium, a akcja GitHub przetłumaczy nowe klucze, tworząc pull request lub zatwierdzając zmiany.
To oszczędza dużo czasu i wysiłku, zachowując jednocześnie wysoką jakość tłumaczeń.
...
Dodaj następujące sekrety w ustawieniach repozytorium (Ustawienia > Sekrety i zmienne > Akcje):
...
- `GITHUB_TOKEN` (Wymagane): Automatycznie dostarczany przez GitHub, używany do tworzenia PR

### Przykładowy plik roboczy
...
```
 ...
 Ten plik roboczy:
- Będzie uruchamiany przy przesyłaniu do głównej gałęzi
- Będzie uruchamiany przy pull requestach do głównej gałęzi
- Może być wyzwalany ręcznie z zakładki Akcje
...
Opcja `create-pull-request` kontroluje, czy akcja tworzy pull request z zmianami, czy zatwierdza je bezpośrednio do gałęzi:

```typescript title=".github/workflows/languine.yml"
with:
  create-pull-request: true # utwórz pull request z zmianami
```

Gdy jest ustawiona na `true`, akcja:
- Utworzy nową gałąź z zmianami
